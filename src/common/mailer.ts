...
export async function sendMail(opts: Options) {
  return await transporter.sendMail({
    from: `Rugin ${env.SMTP_FROM}`,
    to: opts.to.toLowerCase().trim(),
    subject: `${opts.subject} - Rugin`,
    html: opts.body,
    text: opts.text,
  });
}
      ...
      <meta content="width=device-width,initial-scale=1" name="viewport"/>
        <body style="color:#fff;font-family:Arial">
          <div style="background:#000;border-radius:8px;padding:5px;width:100%;display:inline-block">
            <a href="https://rugin.com" style="text-decoration:none;color:#fff">
              <center>
                <img src="https://rugin.com/assets/logo.png" alt="Rugin Logo" style="width:80px;margin-top:20px;margin-bottom:10px"/>
                <h1 style="font-size:24px">Rugin</h1>
              </center>
            </a>
